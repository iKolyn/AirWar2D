import { _decorator, Canvas, CCFloat, CCInteger, Component, director, instantiate, math, Node, Pool, Prefab, UITransform, Vec2, view } from 'cc';
...
export class EnemyManager extends Component {
    //需要飛機生成的頻率
    //注意基本型別可以不用()，但是對象型別需要()
    @property({ type: [CCFloat], tooltip: "Enemy0生成的頻率" })
    ...
    enemySpawn(type: number) {
        ...
        const enemy = instantiate(this.enemyPrefabs[type]);//生成敵人
        const enmyHeight = enemy.getComponent(UITransform).height * 0.5;
        //丟入父節點
